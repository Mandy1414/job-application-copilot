...
router.post('/generate-resume', auth_1.requireAuth, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { jobDescription, userProfile, style = 'professional' } = req.body;
    if (!userProfile) {
        throw (0, errorHandler_1.createError)('User profile is required', 400);
    }
    ...
    `;
        ...
        if (!generatedContent) {
            throw (0, errorHandler_1.createError)('Failed to generate resume content', 500);
        }
        ...
        throw (0, errorHandler_1.createError)(`AI generation failed: ${error.message}`, 500);
    }
}));
// Generate cover letter content
router.post('/generate-cover-letter', auth_1.requireAuth, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { jobDescription, company, position, userProfile } = req.body;
    if (!company || !position) {
        throw (0, errorHandler_1.createError)('Company and position are required', 400);
    }
    // Mock response if OpenAI is not configured
    if (!openai) {
...
${req.user.firstName} ${req.user.lastName}`;
    ...
    `;
        ...
        const generatedContent = completion.choices[0]?.message?.content;
        if (!generatedContent) {
            throw (0, errorHandler_1.createError)('Failed to generate cover letter content', 500);
        }
        res.json({
            success: true,
...
router.post('/optimize-keywords', auth_1.requireAuth, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { jobDescription, resumeContent } = req.body;
    if (!jobDescription) {
        throw (0, errorHandler_1.createError)('Job description is required', 400);
    }
    // Mock response if OpenAI is not configured
    if (!openai) {
        const mockKeywords = {
    ...
    `;
        ...
        const generatedContent = completion.choices[0]?.message?.content;
        if (!generatedContent) {
            throw (0, errorHandler_1.createError)('Failed to analyze keywords', 500);
        }
        // Try to parse JSON response
        let keywordData;
        try {
        ...
        throw (0, errorHandler_1.createError)(`AI analysis failed: ${error.message}`, 500);
    }
}));
